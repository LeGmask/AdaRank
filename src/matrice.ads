generic
  ...
  with function "+"(Gauche : in T_Valeur; Right : in T_Valeur) return T_Valeur;
  with function "*"(Gauche : in T_Valeur; Right : in T_Valeur) return T_Valeur;
package Matrice is
  type T_Dim is array(1..2) of Integer;
  type T_Matrice is array(Positive range <>, Positive range <>) of T_Valeur;

  procedure Init(Mat: out T_Matrice; Val: in T_Valeur);
  ...
  function "*"(A, B: in T_Matrice) return T_Matrice with
    ...
    Post => "*"'Result'Length(1) = A'Length(1) and "*"'Result'Length(2) = B'Length(2);

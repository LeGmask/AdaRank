package body Trifusion is
   ...
   procedure Tri(Vecteur: in out T_Matrice; Ordre : out T_Matrice) is

      procedure Tri_Recursif(Vecteur: in out T_Matrice; Deb, Fin: in Integer) is

         procedure Fusion(Vecteur: in out T_Matrice; Deb, Milieu, Fin: in Integer) is
         I , J : Integer;
         ...
         begin
            ...
            while (I + Deb < Milieu) and then (J + Milieu <= Fin) loop
               if Vecteur (1,J + Milieu) < Vecteur (1,I + Deb) then
                  ...
                  Ordre (1,Deb + I + J) := Ordre (1,I + Deb);
                  I := I + 1;
               else
                  Vecteur_trie (1,Deb + I + J) := Vecteur (1,J + Milieu);
                  Ordre (1,Deb + I + J) := Ordre (1,J + Milieu);
                  J := J + 1;
               end if;
            end loop;
